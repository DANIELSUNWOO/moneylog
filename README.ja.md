...
このうち多く（OIDC、IaC、可観測性、バックアップ／復旧リハーサル）は、一般的な新卒のポートフォリオではあまり扱われない、実際の運用経験がなければ出てこない項目です。各ステップをなぜ教材の範囲を超えて広げたのか、そして費用と時間の制約の中でどのトレードオフを選んだのかは [devops-roadmap.md](docs/devops-roadmap.md) にまとめています。
