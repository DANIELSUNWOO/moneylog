...
명명 규칙: `DUPLICATE_*`(중복), `*_NOT_FOUND`(없음), `*_ERROR`(처리 실패). 도메인별 enum이 `ErrorCode` 인터페이스를 구현한다. (이전 팀 프로젝트에서 쓰던 패턴을 재사용하되 필드명만 `errorCode`→`code`)
...
| D-11 | 검증 실패 시 **필드별 사유**를 `data`에 | `VALIDATION_ERROR` + 메시지 한 줄 | F-07이 "금액>0, 날짜 필수" 같은 구체적 검증을 요구한다. 프론트가 필드 옆에 에러를 표시하려면 어느 필드가 틀렸는지 알아야 한다. (이전 팀 프로젝트에서 아쉬웠던 점을 개선) |
...
- **프론트 스택**: React+Vite(이전 팀 프로젝트 자산 재사용) vs 순수 HTML+fetch — 프론트 셋업 전에 결정.
