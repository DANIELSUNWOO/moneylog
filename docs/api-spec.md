...
- 작성자: 정선우
- 작성일: 2026-09-01 (교안 1-4 대조 후 갱신)
- Base URL: `https://{배포도메인}/api` (로컬: `http://localhost:8080/api`)
- 관련 문서: [요구사항](./requirements.md) · [도메인·ERD](./erd.md) · [DDL](./schema.sql) · [1차 프로젝트 참고](./reference-p2g.md)
...
**검증 실패**는 `data`에 필드별 사유를 담는다. (교안 확장 — D-11)
```json
{
...
**화면은 3종이다.** 교안 흐름도는 4종(로그인/목록/등록/통계)이지만, 우리는 requirements.md D-1에서 **목록과 등록을 한 화면으로 통합**하기로 했다. 등록·수정은 목록 화면 안의 폼(또는 모달)으로 뜬다.
...
| 순위 | 묶음 | API | 이유 | 일차 |
|:--:|------|-----|------|------|
| 1 | 카테고리 | `GET/POST/PUT/DELETE /categories` | 거래가 카테고리를 참조하므로 먼저 | 2일차 |
| 2 | 거래 CRUD | `GET/POST/PUT/DELETE /transactions` | 앱의 핵심 데이터 | 2일차 |
| 3 | 인증 | `signup`, `login` | 토큰이 있어야 나머지를 인증 상태로 테스트 | 3일차 |
| 4 | 인가 | 전 엔드포인트에 user_id 조건 적용 | 인증이 붙어야 "내 것" 개념이 생김 | 3일차 |
| 5 | 목록 필터·페이징 | `GET /transactions?...` 고도화 | CRUD가 된 뒤 조회 조건 확장 | 3일차 |
| 6 | 통계 | `GET /statistics/monthly` | 거래가 쌓여야 집계 의미가 있음 | 3일차 |
| 7 | 프론트 연동 | (화면 3종) | 위 API가 준비된 뒤 | 4일차 |
| 8 | (도전) | `budgets` / `search` / `export` | 기본 완료 후 가점 | 4~5일차 |

교안은 인증을 1순위로 두지만, **머니로그의 일차 일정(2일차=도메인/CRUD, 3일차=인증·인가)에 맞추면 카테고리·거래가 먼저**다. 2일차에는 인증 없이 CRUD를 만들고 H2/MySQL로 동작을 확인한 뒤, 3일차에 인증·인가를 얹으면서 모든 쿼리에 `user_id` 조건을 넣는 순서가 된다. 교안의 순서표에도 카테고리·거래가 2일차, 인증이 3일차로 적혀 있어 **번호만 다르고 일정은 같다.**
...
F-08(프론트 화면)은 7장의 화면 흐름, F-10(배포)은 4일차 작업이다.

---

## 10. 설계 결정 기록

### 10-1. 교안을 따르기로 한 것 (제 초안을 수정)

| 항목 | 초안 | 채택 | 이유 |
|------|------|------|------|
| 통계 경로 | `/stats/monthly` | **`/statistics/monthly`** | 교안이 기준. 축약보다 명확한 편이 낫다. |
| 페이징 위치 | `data` 안에 PageResponse | **`data` + `meta.pagination` 분리** | 목록 데이터와 페이지 상태가 섞이지 않는다. 교안 논리가 더 낫다. |
...
| 에러 코드명 | `EMAIL_DUPLICATED` | **`DUPLICATE_EMAIL`** | 교안 네이밍이 영어 어순으로 더 읽힌다. `DUPLICATE_*` / `*_NOT_FOUND`로 규칙 통일. |
| 통계 필드명 | `totalIncome`/`totalExpense` | **`income`/`expense`/`balance`** | 교안이 기준. 짧고 충분히 명확하다. |
...
| ID | 결정 | 교안 | 근거 |
|----|------|------|------|
| **D-4** | 남의 리소스는 **404** | 403 Forbidden | 403은 "그 id는 존재한다"를 알려준다. id를 훑으면 남의 거래 개수를 추론할 수 있다. 게다가 우리는 **모든 쿼리를 `user_id`로 필터**하기로 했으므로, 남의 행은 애초에 조회 결과에 없다 → 404가 **자연스럽게 나오고 코드도 더 적다**. 403을 주려면 오히려 user_id 없이 조회한 뒤 소유자를 비교하는 단계를 따로 넣어야 한다. 교안도 "404 전략이 흔하다"고 인정하며 "이해하기 쉬워서" 403을 택했다고 밝히고 있다. *(403으로 되돌리려면 예외 매핑 한 줄만 바꾸면 된다.)* |
...
| D-11 | 검증 실패 시 **필드별 사유**를 `data`에 | `VALIDATION_ERROR` + 메시지 한 줄 | F-07이 "금액>0, 날짜 필수" 같은 구체적 검증을 요구한다. 프론트가 필드 옆에 에러를 표시하려면 어느 필드가 틀렸는지 알아야 한다. (P2G의 아쉬운 점 5번 개선) |
...
- **프론트 스택**: React+Vite(P2G 자산 재사용) vs 순수 HTML+fetch — 1-5 셋업 전에 결정.
- **Refresh Token**(F-14, 도전): 현재는 accessToken만. 도입 시 `POST /auth/reissue`와 쿠키 정책 추가.
- **토큰 저장 위치**: localStorage(교안 방식, 구현 단순) vs httpOnly 쿠키(XSS에 강함). 3일차에 결정.
- **CORS**: 프론트를 별도 nginx 컨테이너로 띄우므로 배포 도메인 기준 허용 목록 필요. 4일차에 결정.
