...
스키마를 짜며 직접 판단해야 했던 지점과, 초안 DDL을 검토하며 발견한 문제에 대한 결정.
...
| D-4 | budgets 중복 방지 | **`(user_id, category_id, budget_month)` 유니크 + 애플리케이션 검증** | 유니크 키가 필요한데 초안 DDL에는 빠져 있었다. 다만 MySQL은 NULL을 서로 다른 값으로 취급하므로, `category_id IS NULL`인 "전체 예산"은 유니크 키만으로 중복이 막히지 않는다. 전체 예산 중복은 서비스 레이어에서 검증한다. (대안: 생성 컬럼 `IFNULL(category_id,0)`에 유니크 — schema.sql 주석 참조) |
