...
Terraform은 이 세 가지를 "인프라를 코드로 선언하고, 코드와 실제 상태의 차이를 기계가 계산해서 보여주는" 방식으로 해결하는 도구다(Infrastructure as Code, IaC). AWS·GCP·Azure를 가리지 않고 가장 널리 쓰이는 도구이기도 해서, 해커톤 지원서에도 "인프라를 코드로 관리한다"는 근거로 쓸 수 있다.
